#cNH3 model for the mesophilic and thermophilic stage
import pandas as pd
...
search_spaces = {
    'C': Real(1e-1, 4.0, "log-uniform"),
    'epsilon': Real(0.001, 0.01, "uniform"),
    'kernel': Categorical(['rbf', 'linear']),
    'gamma': Real(1e-1, 0.25, "log-uniform"),
}
...
bayes_cv_tuner = BayesSearchCV(
    ...
    n_iter=50,
    n_jobs=-1,
    cv=GroupKFold(n_splits=10),  # GroupKFold ensures groups are not split across folds
...
# 保存模型
joblib.dump(best_svr, 'SVR_model_12.pkl')
...
def optimize_mlp(trial):
    params = {
        'hidden_layer_sizes': trial.suggest_categorical('hidden_layer_sizes', [(50,), (50, 25), (100,), (100, 50)]),
    ...
    if params['solver'] == 'sgd':
        params['learning_rate_init'] = trial.suggest_float('learning_rate_init', 0.01, 0.06, log=True)
...
joblib.dump(best_mlp, 'MLP_model_12.pkl')
...
joblib.dump(best_et, 'ET_model_12.pkl')
...
search_spaces = {
    ...
    'l2_leaf_reg': Real(0.1, 10.0, "log-uniform"),
}
...
joblib.dump(best_catboost, 'CatBoost_model_12.pkl')
...
gpr_search_spaces = {
    'alpha': Real(1e-5, 1e-1, 'log-uniform'),  # Added noise level
...
joblib.dump(best_gpr, 'GPR_model_12.pkl')
...
# SVR model optimization with group-based cross-validation
svr_model = SVR()
...
# Perform Bayesian optimization (pass groups to fit method)
bayes_cv_tuner.fit(rescaledX_train, y_train, groups=groups_train)
