#cNH3 model for the whole stage
import pandas as pd
...
search_spaces = {
    'C': Real(1e-1, 100.0, "log-uniform"),
    'epsilon': Real(0.01, 1.0, "uniform"),
    'kernel': Categorical(['rbf', 'linear']),
    'gamma': Real(1e-3, 0.2, "log-uniform"),
}
...
def optimize_mlp(trial):
    ...
    if params['solver'] == 'sgd':
        params['learning_rate_init'] = trial.suggest_float('learning_rate_init', 0.01, 0.05, log=True)
...
joblib.dump(best_et, 'ET_model.pkl')
...
joblib.dump(best_gpr, 'GPR_model.pkl')
...
pre_train = best_mlp.predict(rescaledX_train)
print('\nTraining Performance (MLP):')
print('MSE: %.4f' % mean_squared_error(y_train, pre_train))
print('MAE: %.4f' % mean_absolute_error(y_train, pre_train))
print('R2: %.4f' % r2_score(y_train, pre_train))

pre_test = best_mlp.predict(rescaledX_test)
print('\nTesting Performance (MLP):')
print('MSE: %.4f' % mean_squared_error(y_test, pre_test))
print('MAE: %.4f' % mean_absolute_error(y_test, pre_test))
print('R2: %.4f' % r2_score(y_test, pre_test))